...
\newcommand{\mataHello}{{\tilde{E}\mkern-10mu\raise1pt\hbox{$/\mkern-8mu/$}}\mkern2mu\hat{y}}
...
\NewDocumentEnvironment{horizontable}{ s O{c} O{|} +v }
{%
\numdef\colcount{\getarraylength{#4}}
\typeout{ampcount is: \colcount}%
\retokenize\headerrow{#4}%
\IfBooleanTF{#1}{\def\colspec{#3}}{\def\colspec{}}%
\multido{}{\colcount}{%
	\gappto\colspec{#3#2}
}%
\IfBooleanTF{#1}{\gappto\colspec{#3#3}}{\gappto\colspec{#3}}%
\expandafter\tabular\expandafter{\colspec}
...
A table of the diacritic marks follows:
\begin{table}[tbh!]
%\centering
\begin{tabularx}{13em}{|| >{\centering\arraybackslash}m{3em} | >{\centering\arraybackslash}m{7em} ||}
\specialrule{.2em}{1em}{0em}
\hline
Symbol & Equivalent vowel \\
\hline
$\hat{\circ}$ & a \\
\hline
$\ddot{\circ}$ & e \\
...
$?$ & ? & $br$ & Convert an operator to a question \\ \hline
...
\\
\\
The ordinal variants are as follows: \vspace{0.25em}\\
%\begin{stdtable}{6}%\begin{tabular}{||c|c|c|c|c|c||}
%\hline
%Number & 1 & 2 & 3 & 4 & 5 \\ \hline
%Word & $a\breve{k}_\prime$ & $\ddot{l}$ & $\dot{d}r$ & $\breve{f}_\prime$ & $\breve{k}n$ \\ \hline
%\end{stdtable}
\begin{horizontable}*
{Number & 1 & 2 & 3 & 4 & 5}
Word & $a\breve{k}_\prime$ & $\ddot{l}$ & $\dot{d}r$ & $\breve{f}_\prime$ & $\breve{k}n$ \\ \hline
...
\emph{Mata} supports most of the same pronouns as \emph{English} but remember that objects like pronouns are sets which can be combined together to describe different groups.\\
\begin{wordtabular}{5}
I & $\mathcal{E}$ & You & $\mho$ & they/it & $\tilde{g}$ & You(p)$^\dagger$ & $p\hat{y}$ & they(p)$^\ddagger$ & $g\hat{y}$ \\\hline
...
\subsubsection{Nouns}
\begin{wordtabular}{5}
Past & $\tilde{u}p$ & Future & $k\breve{l}$ & Name & $\hat{n}_\prime m$ & Night & $n\dot{y}c$ & Apple & $a\breve{p}l$ \\\hline
\end{wordtabular}
\end{document}